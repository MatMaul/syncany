...
public abstract class SignatureTransformer extends Transformer implements ISigner, IVerifier {
	public static final String PROPERTY_SIGNING_KEY = "signingkey";
	...
	public void init(Map<String, String> settings) throws Exception {
		String signingKeyStr = settings.get(PROPERTY_SIGNING_KEY);
		String verifyKeyStr = settings.get(PROPERTY_VERIFY_KEY);

		if (verifyKeyStr == null && signingKeyStr == null) {
			throw new Exception("Setting '"+PROPERTY_VERIFY_KEY+"' or '"+PROPERTY_SIGNING_KEY+"' must be filled.");
		}

		init(signingKeyStr == null ? null : StringUtil.fromHex(signingKeyStr), verifyKeyStr == null ? null : StringUtil.fromHex(verifyKeyStr));
	}

	protected abstract void init(byte[] signingKey, byte[] verifyKey);
